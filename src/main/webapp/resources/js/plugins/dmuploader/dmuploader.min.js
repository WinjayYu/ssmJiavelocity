...
(function(t) {
    ...
    i.prototype.init = function() {
        ...
        e.element.find("input[type=file]").on("change",
        function(n) {
    ...
    i.prototype.queueFiles = function(e) {
        ...
        for (var r = 0; r < e.length; r++) {
            ...
            if (this.settings.maxFileSize > 0 && i.size > this.settings.maxFileSize) {
                this.settings.onFileSizeError.call(this.element, i);
                continue
            }
            if (this.settings.allowedTypes != "*" && !i.type.match(this.settings.allowedTypes)) {
                this.settings.onFileTypeError.call(this.element, i);
                continue
            }
            if (this.settings.extFilter != null) {
                var s = this.settings.extFilter.toLowerCase().split(";");
                var o = i.name.toLowerCase().split(".").pop();
                if (t.inArray(o, s) < 0) {
            ...
            if(!this.settings.onFilterJudge.call(this.element, i)){
            	continue
            }
            
            this.queue.push(i);
            var u = this.queue.length - 1;
            this.settings.onNewFile.call(this.element, u, i)
        }
    ...
    i.prototype.processQueue = function() {
        ...
        if (n.queuePos >= n.queue.length) {
            n.settings.onComplete.call(n.element);
            n.queuePos = n.queue.length - 1;
            n.queueRunning = false;
            return
        }
        var r = n.queue[n.queuePos];
        var i = new FormData;
        i.append(n.settings.fileName, r);
        n.settings.onBeforeUpload.call(n.element, n.queuePos);
        t.each(n.settings.extraData,
        function(e, t) {
            i.append(e, t)
        });
        n.queueRunning = true;
    ...
    t(document).on("dragenter",
    function(e) {
        e.stopPropagation();
        e.preventDefault()
    });
    t(document).on("dragover",
    function(e) {
        e.stopPropagation();
        e.preventDefault()
    });
