...
(function ($, window, document, Date) {
    ...
    var _markup =
        '<div class=" month-picker-header">' +
            '<table class="month-picker-year-table">' +
                '<tr >' +
                    '<td class="month-picker-previous"><a class="ui-datepicker-prev" /></td>' +
                    '<td class="month-picker-title" style="margin-right: 0px;"><a /></td>' +
                    '<td style="margin-left: -30px">年</td>' +
    ...
    $.widget("KidSysco.MonthPicker", {
        ...
        _create: function () {
            ...
            var _menu = this._monthPickerMenu = $('<div id="MonthPicker_' + _el[0].id + '" class="month-picker ui-widget ui-widget-content "></div>').hide();
            ...
            //todo xg override childNode
            this._nextButton.html('<span class="ui-icon ui-icon-circle-triangle-e"></span>')
            this._prevButton.html('<span class="ui-icon ui-icon-circle-triangle-w"></span>')
        ...
        _chooseMonth: function (month) {
            ...
            _event('OnAfterChooseMonth', this)(_year,month);
        },
        ...
        _onMonthClick: function(event) {
            var disable=$(event.currentTarget).hasClass('ui-button-disabled');
            if (!disable){//xg add
        ...
        _decorateButtons: function() {
            ...
            _setDisabled(this._prevButton,  false);//不限制最小年份
            // _setDisabled(this._nextButton,  _maxDate && _curYear == _toYear(_maxDate));
            var disable=_curYear>=new Date().getFullYear();
            _setDisabled(this._nextButton, disable );//最大年份是当年
            // if (disable){
            //     //todo 不能直接接触绑定。。。
            // }
