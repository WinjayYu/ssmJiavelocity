(function ($, undefined) {
    ...
    $.widget('lugolabs.weekpicker', {
        ...
        _create: function () {
            ...
            $(document)
                .on('mousemove', '.ui-datepicker-calendar tr', function () {
                    ...
                    var len=$(this).parent().parent().find('th').length;
                    if (len===8){//判断是否是日然周,防止污染自然日
        ...
        _select: function (dateText, inst, onSelect) {
            ...
            if (onSelect) onSelect(startDateText, endDateText);
        },
        ...
        _setWeek: function (date) {
            ...
            this._startDate = new Date(year, month, day + 1);
            this._endDate = new Date(year, month, day + 7);
        },
